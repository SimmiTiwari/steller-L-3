...
- **⚙️ CI/CD Pipeline**: Configured via GitHub Actions (.github/workflows/ci.yml)
...
The project includes a robust, pre-configured CI/CD pipeline defined in [.github/workflows/ci.yml](file:///.github/workflows/ci.yml) that runs automatically on every push or pull request to the `main` or `master` branches.

### Automated Verification Steps
1. **Smart Contract (Soroban/Rust)**:
   - Sets up the Rust toolchain with the `wasm32-unknown-unknown` target.
   - Automatically resolves `arbitrary` package dependencies to fix transitively derived compile errors on newer toolchains.
   - Runs smart contract unit tests: `cargo test`.
   - Verifies release-ready WASM compilation: `cargo build --target wasm32-unknown-unknown --release`.
2. **Frontend (Vite/React)**:
   - Sets up Node.js environment.
   - Automatically installs and audits npm package dependencies.
